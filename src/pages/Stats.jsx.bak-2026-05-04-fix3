...
function Stats() {
  ...
  // Reset minRounds to sensible defaults when team filter changes
  useEffect(() => {
    if (teamFilter === 'jv') {
      setMinRounds(1)
    } else {
  ...
  // Sorted leaderboard players (with minRounds filter)
  const sortedPlayers = useMemo(() => {
    const getValidScores = (p) => p.scores.filter(s => s !== null && s > 50)
    return [...filteredPlayers]
      .filter(p => !p.isMatchPlayOnly)
      .filter(p => getValidScores(p).length >= minRounds)
      .sort((a, b) => {
        if (sortCol === 'player') {
          return sortDir === 'asc'
  ...
  }, [filteredPlayers, sortCol, sortDir, minRounds])
                ...
                <div className="flex flex-wrap items-center justify-between gap-3 p-4 border-b border-gray-200">
                  ...
                  <div className="flex items-center gap-3 flex-wrap">
                    ...
                    <div className="flex items-center gap-1">
                      <span className="text-xs text-gray-500 font-medium">Min Rounds:</span>
                      <div className="flex space-x-0.5 bg-gray-100 p-0.5 rounded">
                        {[1, 3, 5].map(n => (
                          <button
                            key={n}
                            onClick={() => setMinRounds(n)}
